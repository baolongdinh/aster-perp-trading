...
func (h *DefensiveStateHandler) isRecoveryOpportunity(
	...
	// Recovery: defensive deleveraging succeeded and price has regained
	// structure with at least a modest positive excursion from the entry zone.
	if unrealizedPnL > 0 && currentPrice >= entryPrice*(1+h.breakevenThreshold*2) {
		return true
	}
