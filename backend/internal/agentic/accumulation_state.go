...
func (h *AccumulationStateHandler) detectWyckoffPhase(
	...
	elapsed := time.Since(h.accumulationTime[symbol])
	priceChangePct := 0.0
	if entryPrice > 0 {
	...
	case PhasePreliminarySupport:
		// Stay in preliminary support until enough time has passed and price compresses lower.
		if elapsed > 20*time.Minute && bbWidth < 0.018 && adx < 18 && priceChangePct <= -0.01 {
			h.wyckoffPhase[symbol] = PhaseSellingClimax
		}

	case PhaseSellingClimax:
		if elapsed > 45*time.Minute && priceChangePct > -0.015 {
	...
	case PhaseSecondaryTest:
		if currentPrice < entryPrice*0.98 && bbWidth < 0.025 {
			h.wyckoffPhase[symbol] = PhaseSpring
		}

	case PhaseSpring:
		if bbWidth < 0.015 && adx > 18 {
			h.wyckoffPhase[symbol] = PhaseBreakout
		}

	case PhaseBreakout:
		if currentPrice > entryPrice*(1+h.breakoutThreshold) && adx > 30 {
...
func (h *AccumulationStateHandler) isSignOfStrength(
	...
	if regimeSnapshot.ADX >= 40 && regimeSnapshot.BBWidth >= 0.06 && regimeSnapshot.Confidence >= 0.75 {
		return true
	}
