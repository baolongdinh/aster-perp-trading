#!/usr/bin/env bash
# Run script for Volume Farm Maker (Micro Profit Engine)
# Usage: ./run-volume-maker.sh [start|stop|restart|status|logs|build]

set -e

# Configuration
APP_NAME="volume-maker"
CMD_DIR="./cmd/volume-farm-maker"
CONFIG_FILE="${2:-./config/volume-farm-config.yaml}"
LOG_FILE="volume-maker.log"
PID_FILE="volume-maker.pid"
...
start() {
    if [ -f "${PID_FILE}" ] && kill -0 "$(cat "${PID_FILE}")" 2>/dev/null; then
        log_warn "Volume Maker bot is already running (PID: $(cat "${PID_FILE}"))"
        exit 0
    fi
    ...
    if [ ! -f "${CONFIG_FILE}" ]; then
        ...
        log_info "Create one from: cp config/volume-farm-config.yaml.example config/volume-farm-config.yaml"
        exit 1
    fi

    acquire_wake_lock

    log_info "Starting Volume Farm Maker with config: ${CONFIG_FILE}"
    log_info "Features: Maker strategy only, ETHUSD1, spread: 5 bps, leverage: 50x"
    ...
    if kill -0 ${PID} 2>/dev/null; then
        log_success "Volume Maker bot started successfully (PID: ${PID})"
...
stop() {
    if [ ! -f "${PID_FILE}" ]; then
        ...
        if [ -n "${PID}" ]; then
            log_info "Stopping Volume Maker bot processes..."
            kill ${PID} 2>/dev/null || true
            log_success "Bot stopped"
        else
            log_warn "No Volume Maker bot process found"
        fi
        release_wake_lock
        return 0
    fi
    ...
    if kill -0 "${PID}" 2>/dev/null; then
        ...
        log_success "Volume Maker bot stopped"
    else
...
restart() {
    log_info "Restarting Volume Maker bot..."
    stop
    sleep 1
    start
}
...
status() {
    if [ -f "${PID_FILE}" ]; then
        PID=$(cat "${PID_FILE}")
        if kill -0 "${PID}" 2>/dev/null; then
            log_success "Volume Maker bot is running (PID: ${PID})"
            ...
            if [ -f "${LOG_FILE}" ]; then
                ...
                tail -n 5 "${LOG_FILE}" | grep -E "(INFO|ERROR|WARN|Starting|Stopped)" | tail -n 3 || true
            fi
        else
        ...
        if [ -n "${PID}" ]; then
            ...
            log_warn "Volume Maker bot is not running"
        fi
    fi
}
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  start    - Start Volume Maker with wake lock"
    ...
    echo "      The bot uses Maker strategy only (place limit orders)."
}
